...
def zhed(request):
   ...
   # Ensure session variables are properly initialized
    ...
    if "all_guessed" not in request.session:
        request.session["all_guessed"] = False  # Initialize all_guessed in session
    ...
    if "letter" in request.POST:
        ...
        all_guessed = set(word).issubset(set(correct_guesses))
    ...
    return render(request, 'Zhed.html', {
        ...
        'all_guessed': request.session["all_guessed"]  # Pass all_guessed to the template
            })


cgw = [char if char in correct_guesses else "__" for char in word]
